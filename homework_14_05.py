...
async def info(message):
    with open(f'R2-D2.jpeg', 'rb') as img:
...
async def get_buying_list(message):
    for idx in range(4):
        await message.answer(
            f'Название: {get_all_products()[idx][1]} | Описание: {get_all_products()[idx][2]} | Цена: {get_all_products()[idx][3]}')
        with open(f'product_{idx + 1}.jpg', 'rb') as img:
            await message.answer_photo(img)
...
async def all_message(message):
    await message.answer(f'Нажмите /start, чтобы открыть меню.')
